...
public class InsertIntervals {
    public static void main(String[] args) {
        ...
        System.out.println(Arrays.toString(insert(interval, newInterval)));
    }
    ...
    public static int[][] insert(int[][] intervals, int[] newInterval) {
        ...
        while (idx < intervals.length) {
            ...
            if (intervals[idx][0] < newInterval[0]) {
                ...
                // or at the same time as the new interval, exit the loop
            } else {
                break;
            }
        }
        // If the answer list is empty or the last interval in the answer
        // list ends before the new interval starts
        // add the new interval to the answer list
        if (ans.size() == 0 || (newInterval[0] > ans.get(ans.size() - 1)[1])) {
            ans.add(newInterval);
            // If the last interval in the answer list overlaps with the new interval,
        ...
        while (idx < intervals.length) {
            int[] lastInterval = ans.get(ans.size() - 1);
            // If the last interval in the answer list overlaps with the current interval, merge them by updating
